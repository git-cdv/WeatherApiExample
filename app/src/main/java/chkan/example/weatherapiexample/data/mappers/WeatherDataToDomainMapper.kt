    ...
    override fun map(input: WeatherDataModel): WeatherDomainModel {
        return WeatherDomainModel(
            ...
            forecastListByDay = input.forecast?.forecastList?.map { item ->
                ForecastWeatherItem(date = item.date.createDate(), maxTempC = item.day?.maxtempC.orEmpty(), minTempC = item.day?.mintempC.orEmpty(), iconUrl = item.day?.conditionWithIcon?.icon.createIcon())
            } ?: emptyList()
        )
    }
